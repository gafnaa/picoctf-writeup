# Sum-O-Primes

## Desc

We have so much faith in RSA we give you not just the product of the primes, but their sum as well!
- gen.py
- output.txt

## Solution

Solve it with this code
```py
from binascii import unhexlify
import math

# Given values from output.txt
x_hex = ''
n_hex = ''
c_hex = ''

# Convert from hex to integers
x = int(x_hex, 16)
n = int(n_hex, 16)
c = int(c_hex, 16)
e = 65537

# Calculate p and q by solving the quadratic equation x^2 - x(p+q) + pq = 0
# Which is x^2 - x*x + n = 0 => x = [x ± sqrt(x^2 - 4n)] / 2
discriminant = x*x - 4*n
sqrt_discriminant = math.isqrt(discriminant)

# Check if the discriminant is a perfect square
if sqrt_discriminant * sqrt_discriminant != discriminant:
    print("Discriminant is not a perfect square!")
    exit()

p = (x + sqrt_discriminant) // 2
q = (x - sqrt_discriminant) // 2

# Verify p and q
assert p * q == n
assert p + q == x

# Compute phi(n) = (p-1)*(q-1)
phi = (p - 1) * (q - 1)

# Compute modular inverse of e mod phi
d = pow(e, -1, phi)

# Decrypt the ciphertext
m = pow(c, d, n)

# Convert the message to bytes
flag_bytes = unhexlify(hex(m)[2:])
print(flag_bytes.decode())
```

1. First, we convert all the given hexadecimal values to integers.
2. We know that for any two numbers p and q:
   * p + q = x
   * p * q = n

3. We can solve for p and q using the quadratic formula derived from these equations.

4. The `discriminant (x² - 4n)` must be a perfect square for p and q to be integers.

5. Once we have p and q, we can compute `φ(n) = (p-1)(q-1)`.

6. We then compute the private exponent d as the modular inverse of `e modulo φ(n)`.

7. Finally, we decrypt the ciphertext using `m = cᵈ mod n `and convert the result to a string.
   
## Flag
    picoCTF{pl33z_n0_g1v3_c0ngru3nc3_0f_5qu4r35_3921def5}